    ...
    <div className="max-w-md mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">Valida cupones de clientes</p>
        </div>
      </div>
      ...
      {!scanning && !result && !processing && (
        <div className="card p-8 text-center">
          ...
          <h2 className="font-semibold text-gray-900 mb-2">Listo para escanear</h2>
      ...
      {processing && (
        <div className="card p-8 text-center">
          ...
          <p className="text-gray-600 font-medium">Validando cupón...</p>
        </div>
      )}
        ...
        <div className={`card p-6 ${result.success ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}>
          <div className="flex flex-col items-center text-center gap-3">
            ...
            {result.success && result.coupon && (
              <div className="w-full bg-white rounded-xl p-4 mt-2 text-left">
                <p className="font-semibold text-gray-900">{result.coupon.title}</p>
                ...
                <p className="text-xs text-gray-500 font-mono mt-1">{result.coupon.code}</p>
              </div>
            )}

            <button onClick={reset} className={`w-full mt-2 py-2.5 rounded-xl font-semibold text-sm transition-colors ${result.success ? "bg-green-600 hover:bg-green-700 text-white" : "bg-red-600 hover:bg-red-700 text-white"}`}>
              Escanear otro cupón
            </button>
          </div>
        </div>
